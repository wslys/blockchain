...
class BinanceApi
{
    protected $base = 'https://api.binance.com/api/';        // < REST endpofor the currency exchange
    protected $wapi = 'https://api.binance.com/wapi/';       // < REST endpofor the withdrawals
    protected $stream = 'wss://stream.binance.com:9443/ws/'; // < Endpofor establishing websocket connections
     ...
     *
     * @param $file the name of the property to return
     * @return null
     */
     ...
     *
     * @param $member the name of the member property
     ...
     * @param $symbol the currency symbol
     * @param $limit the amount of orders returned
     * @param $fromTradeId (optional) return the orders from this order onwards. negative for all
     * @return array with error message or array of orderDetails array
     * @throws \Exception
     */
     ...
     *
     * @param $asset currency such as BTC
    ...
    public function depth($symbol)
    {
        if (isset($symbol) == false || is_string($symbol) == false) {
            // WPCS: XSS OK.
            echo "asset: expected false, " . gettype($symbol) . " given" . PHP_EOL;
        }
    ...
    /**
     * getProxyUriget Uniform Resource Identifier assocaited with proxy config
     *
     ...
     *
     * @param $url the endpoto query, typically includes query string
     * @param $method this should be typically GET, POST or DELETE
     * @param $params array addtional options for the request
     * @param $signed true or false sign the request with api secret
    ...
    private function httpRequest($url, $method = "GET", array $params = [], $signed = false)
    {
        ...
        if ($signed == true) {
            ...
            $endpo= $base . $url . '?' . $query . '&signature=' . $signature;
            curl_setopt($curl, CURLOPT_URL, $endpoint);
            curl_setopt($curl, CURLOPT_HTTPHEADER, array(
                'X-MBX-APIKEY: ' . $this->api_key,
            ));
        }
        // params so buildquery and append to url
    ...
    public function order($side, $symbol, $quantity, $price, $type = "LIMIT", array $flags = [], $test = false)
    {
        ...
        // someone has preformated there 8 decimal podouble already
        ...
        if (is_string($price) === false) {
            // WPCS: XSS OK.
            echo "warning: price expected got " . gettype($price) . PHP_EOL;
        }
        ...
        $q= ($test === false) ? "v3/order" : "v3/order/test";
     ...
     *
     * @param $chart array
     * @param $include_volume for inclusion of volume
     ...
     *
     * $output= $api->displayDepth($array);
     *
     * @param $array array
     * @return of the depth information
     */
     ...
     *
     * @return showing the total transfered
     */
     ...
     *
     * @return get the total number of api calls
     */
     ...
     * //print_r($depth); // Prall depth data
    ...
    public function depthCache($symbols, callable $callback)
    {
        ...
        foreach ($symbols as $symbol) {
            ...
            $endpo= strtolower($symbol) . '@depthCache';
    ...
    public function trades($symbols, callable $callback)
    {
        ...
        foreach ($symbols as $symbol) {
            ...
            $endpo= strtolower($symbol) . '@trades';
            $this->subscriptions[$endpoint] = true;
    ...
    {
        $endpo= $symbol ? strtolower($symbol) . '@ticker' : '!ticker@arr';
        $this->subscriptions[$endpoint] = true;
     ...
     * @throws \Exception
     */
    public function chart($symbols, $interval = "30m", callable $callback)
    {
        if (!is_array($symbols)) {
            $symbols = [
                $symbols,
        ...
        foreach ($symbols as $symbol) {
            ...
            $endpo= strtolower($symbol) . '@kline_' . $interval;
            $this->subscriptions[$endpoint] = true;
    ...
    /**
     * kline Subscribes to @klines WebSocket endpofor latest chart data only
     *
